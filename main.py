...
from app import update_fibery_entity
...
def analyze_video(url, publish=False, fibery_id=None):
    """Analyze video from URL and optionally update Fibery"""
    try:
        ...
        # If we have a Fibery ID, update the entity with results
        if fibery_id:
            update_fibery_entity(fibery_id, results)
            print(f"\n✅ Results updated in Fibery entity: {fibery_id}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--publish', action='store_true', help='Publish results to GitHub')
    parser.add_argument('--fibery-id', help='Fibery entity ID for updating results')

    args = parser.parse_args()
    analyze_video(args.video_url, args.publish, args.fibery_id)